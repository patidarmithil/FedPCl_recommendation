...
── Temperature ─────────────────────────────────────────────────────────
Paper states τ=0.3 but Figure 3 tests τ∈{0.125..0.225}, best at τ=0.2.
We use τ=0.2 (supported by paper's own ablation).
"""
...
def user_contrastive_loss(e0_all: torch.Tensor,
    ...
    Implements paper Eq.(5) exactly:
      L = -log( exp(sim(e_u^0, e_u^l)/τ) / Σ_{v≠u} exp(sim(e_u^0, e_v^0)/τ) )

      anchor/query = e0_all[0]   = e_u^(0)    layer-0 of anchor user
      positive     = el_anchor   = e_u^(l)    even-layer of anchor user
      negatives    = e0_all[1:]  = e_v^(0)    layer-0 of neighbour users
      denominator  = Σ over negatives ONLY (positive NOT in denominator per Eq.5)

    Args:
        e0_all:    [N, d]  layer-0 embeddings: e0_all[0]=anchor, e0_all[1:]=neighbours
        el_anchor: [d]     even-layer embedding of anchor user u
        tau:       float   temperature (paper Fig.3 best at τ∈{0.175,0.2})
    ...
    anchor = e0_norm[0]    # [d]  — e_u^(0), the query/anchor
    negs   = e0_norm[1:]   # [N-1, d] — e_v^(0) for v≠u, the negatives

    # Paper Eq.(5): anchor = e_u^(0), positive = e_u^(l), negatives = e_v^(0)
    # Denominator contains ONLY negatives (not the positive term) — matches paper.
    # Numerator: sim(e_u^0, e_u^l)
    pos_sim  = (anchor * el_norm.squeeze(0)).sum()      # scalar
    # Denominator: Σ_{v≠u} sim(e_u^0, e_v^0)
...
def item_contrastive_loss(E_pos: torch.Tensor,
    ...
    Args:
        ...
        drop_rate: float       dropout probability for augmentation.
                               With d=64, drop_rate=0.3 drops ~19 dims giving
                               cosine(view1,view2) ≈ 0.70 — meaningful contrast.
                               drop_rate=0.1 only drops 6 dims → cosine ≈ 0.94
                               → near-trivial positives → very weak signal.
                               Paper doesn't specify; 0.3 is correct for d=64.
        tau:       float       temperature — paper Section IV-A uses τ=0.3
